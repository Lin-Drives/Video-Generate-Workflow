...
('双目与深度相机','两个摄像头像人的双眼，用视差估计深度。结构光和飞行时间相机，则主动发光去量距离。','这台硅片搬运机器人正对自己储位上的一个透明晶圆盒，深度相机透出克制的浅色光锥，光锥自然落在晶圆盒的透明外壳与层叠晶圆的棱边上，像物理光影而非图形标注。'),
('激光雷达','激光雷达一圈圈扫描，直接给出精确的三维点云。它不怕黑，但贵，也分不清颜色和纹理。','这台硅片搬运机器人在洁净室中缓缓转向，底盘的黑色激光雷达窗口正在扫描，周围空间中悬浮着由细密橙色光点组成的三维点云，点云自然贴合墙面、机台与地面轮廓，写实光影质感。'),
...
('从像素到语义','神经网络把像素变成物体，这是门，那是台阶，前面有人。机器人要知道的不只是有什么，还有能不能走、能不能抓。','这台硅片搬运机器人在一台工艺机台的上下料口前停下，顶部深灰色机械臂的夹爪悬停在对接位置前方，表现它正在分辨这是哪台机台和能不能对接的判断瞬间。'),
...
prompt_doc=['# GPT 图像工作流分镜提示词：机器人怎么"看见"世界？', '', '主配置：GPT 图像工作流；Qwen 仅作备选。', '素材来源：assets/优艾智合1.png 至 assets/优艾智合4.png，即优艾智合（YOUIBOT）OW12-300 十二寸晶圆搬运移动操作机器人的官方素材。外观要点：白色方形低矮移动底盘，底部一圈黑色饰边，车身侧面有青绿色状态灯带和黑色内凹的激光雷达窗口；上部白色机身，正面和侧面布置多个透明开放式晶圆盒储位，盒内可见层叠晶圆；顶部中央搭载深灰色六轴协作机械臂，末端为银色金属夹爪；机身有红色急停按钮与小型触摸屏。生图时严格以参考图为准，不得改成黑色 FOUP 或立柱式结构。', '动态镜头准入：本片为轮式底盘，无步态问题，但行进镜头必须人工检查底盘透视一致性、轮子与地面的接触关系、机械臂与机身的连接位置、透明晶圆盒的层叠结构是否合理；含点云、光锥等可视化元素时，必须保持其像物理光影而非屏幕图形，避免落入示意图风格。', f'统一风格：{image_style}', f'负面提示词：{negative_prompt}', '']
