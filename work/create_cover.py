...
def add_tech_title(canvas, scale=1):
    """Use the first video tech-cover language without changing the approved background."""
    font = ImageFont.truetype(FONT, 142 * scale, index=2)
    lines = ['机器人如何', '看见世界？']
    x, y = 126 * scale, 252 * scale
    ...
    text_y = y
    for line in lines:
        mask_draw.text((x, text_y), line, font=font, fill=255, stroke_width=2 * scale, stroke_fill=255)
        text_y += 166 * scale
    for radius, opacity in ((28, 105), (9, 180)):
        glow = Image.new('RGBA', canvas.size, (18, 129, 255, 0))
        blurred = mask.filter(ImageFilter.GaussianBlur(radius * scale))
    ...
    draw.line(tuple(value * scale for value in (112, 242, 875, 207)), fill=(33, 148, 255, 160), width=3 * scale)
    draw.line(tuple(value * scale for value in (112, 246, 790, 215)), fill=(170, 228, 255, 120), width=scale)
    draw.polygon([(105 * scale, 235 * scale), (142 * scale, 233 * scale), (128 * scale, 248 * scale)], fill=(106, 204, 255, 230))
    draw.polygon([(842 * scale, 209 * scale), (876 * scale, 207 * scale), (862 * scale, 221 * scale)], fill=(106, 204, 255, 190))

    text_y = y
    for line in lines:
        draw.text(
            (x, text_y), line, font=font, fill=(248, 253, 255, 255),
            stroke_width=3 * scale, stroke_fill=(67, 170, 255, 255),
        )
        # Thin offset highlight makes the title feel engineered rather than bubbly.
        draw.text((x + 3 * scale, text_y - 2 * scale), line, font=font, fill=(255, 255, 255, 255))
        text_y += 166 * scale

    draw.polygon([(126 * scale, 603 * scale), (825 * scale, 571 * scale), (830 * scale, 592 * scale), (121 * scale, 624 * scale)], fill=(255, 72, 30, 255))
    for offset in range(0, 70, 14):
        draw.line(tuple(value * scale for value in (740 + offset, 582 - offset // 3, 752 + offset, 577 - offset // 3)), fill=(255, 184, 114, 255), width=2 * scale)
...
def main():
    ...
    if tech_style:
        # Supersample title geometry, then reduce it to the delivery resolution.
        # This keeps the neon outlines clean instead of leaving staircase edges.
        title_scale = 3
        canvas = canvas.resize((W * title_scale, H * title_scale), Image.LANCZOS)
        add_tech_title(canvas, title_scale)
